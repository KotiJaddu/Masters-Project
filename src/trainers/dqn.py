""""""""""""""""""""
#  Trains a deep Q network (reinforcement learning component) to
...
def run_test(env, model, validation):
    """
...
def run(data_path, parameters={"LEARNING_RATE" : conf.RL_LEARNING_RATE,
    ...
    Args:
            ...
            save_model: saves the DQN policy model to disk
        ...
        while not done:
            ...
            if np.random.uniform(0, 1) < exploration_rate:
                ...
                if previous_action is None:
                    ...
                    action = int(torch.argmax(POLICY_NET(torch.tensor(current_state + [previous_action]))))
            ...
            if not len(buffer) < conf.DQN_BATCH_SIZE and step_count % 10 == 0:
                ...
                state_batch, action_batch, nextstate_batch, reward_batch = \
                    (torch.stack(x) for x in zip(*transitions))
                # Compute the loss
                bellman_targets = parameters["DISCOUNTED_FUTURE_REWARD_FACTOR"][instrument] * \
                    (TARGET_NET(nextstate_batch)).max(1).values + reward_batch.reshape(-1) 
    ...
    if verbose:
        print("Validation")
    rewards_per_episode_v, drawdown_per_episode_v = run_test(env, POLICY_NET, validation=True)
    if verbose:
    ...
    if verbose:
        print("Testing")
    rewards_per_episode_t, drawdown_per_episode_t = run_test(env, POLICY_NET, validation=False)
    if verbose:
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m trainers.dqn path_to_ofi_data")
        sys.exit(0)
    run(sys.argv[1])
